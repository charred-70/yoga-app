...
export default function RoutinePage() {
    ...
    const poses = [
        ...
        { id: 3, name: "Downward Dog", image: "/neutral.png", value: "downwardDog" },
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-sky-50 font-sans">
            <div className="w-full pt-16 pb-12 px-6">
                <div className="max-w-7xl mx-auto text-center space-y-4">

                    <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold bg-gradient-to-r from-pink-500 via-purple-300 to-indigo-600 bg-clip-text text-transparent">
                        Create your Routine!
                    </h1>
                    ...
                    {poses.map((pose) => (
                        <div
                            ...
                            className={`group relative cursor-pointer rounded-3xl transition-all duration-300 transform hover:scale-105 ${selectedPoses.includes(pose.id)
                                ? "bg-gradient-to-br from-red-300 to-pink-400 shadow-2xl shadow-pink-300"
                        ...
                        >
                            ...
                            <div className="p-6 space-y-4">
                                {/* Image Container */}
                                <div className={`relative aspect-square w-full rounded-2xl overflow-hidden transition-all duration-300 ${selectedPoses.includes(pose.id)
                                ...
                                {/* Pose Name */}
                                <p className={`text-xl font-semibold text-center transition-colors ${selectedPoses.includes(pose.id)
                                    ? "text-white"
                            ...
                            {!selectedPoses.includes(pose.id) && (
                                <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-pink-400/0 to-purple-500/0 group-hover:from-pink-400/5 group-hover:to-purple-500/5 transition-all duration-300" />
                            )}
                        </div>
                    ))}
                </div>
            ...
            {selectedPoses.length > 0 && (
                <div className="fixed bottom-8 left-0 right-0 flex justify-center px-6 z-50">
                    <Link href="/scoring">
                        <button
                            onClick={sendPose}
                            className="group bg-gradient-to-r from-red-300 to-pink-400 hover:from-red-500 hover:to-pink-500 text-white font-bold py-5 px-10 rounded-full shadow-2xl shadow-pink-300 transform transition-all duration-300 hover:scale-110 flex items-center gap-3 text-lg backdrop-blur-sm"
                        >
                            <div className="bg-white/20 rounded-full p-2">
                                <Check size={24} className="group-hover:rotate-12 transition-transform" />
                            </div>
                            <span>
